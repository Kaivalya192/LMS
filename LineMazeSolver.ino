...
float Kp = 0.02,Kd = 0.02,PIDvalue;
...
void botleft() {
  ...
  analogWrite(motor2PWM, lfspeed-7);
  digitalWrite(motor1A, LOW);
...
void botright() {
  analogWrite(motor1PWM, lfspeed-35);
  analogWrite(motor2PWM, lfspeed-22);
  digitalWrite(motor1A, HIGH);
  digitalWrite(motor1B, LOW);
  digitalWrite(motor2A, LOW);
  digitalWrite(motor2B, HIGH);
...
void PID()
{
  error = (analogRead(1) - analogRead(3));
  if(error>450){
